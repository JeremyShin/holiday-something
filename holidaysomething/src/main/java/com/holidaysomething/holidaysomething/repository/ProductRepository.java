...
public interface ProductRepository extends JpaRepository<Product, Long>, ProductRepositoryCustom {
  ...
  // 소분류 조회하기
  @Query(value = "SELECT new com.holidaysomething.holidaysomething.dto.ProductListImageDto(p.id,p.name,p.sellingPrice,p.originalPrice,p.quantity,p.safeQuantity,p.optionalPriceText,pi.path,pi.storedFileName,pc.id) FROM Product p JOIN FETCH ProductImage pi ON p.id=pi.product.id JOIN FETCH ProductCategory pc ON pc.id=p.productCategory.id WHERE pi.category=1 AND pc.id=:categoryId")
